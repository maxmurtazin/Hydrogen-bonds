...
def main():
    ...
    ap.add_argument('--out_dir', type=str, default='runs/rl_ncg_demo')
    args = ap.parse_args()
    ...
    for ep in range(args.episodes):
        ...
        for t in range(args.horizon):
            ...
            corr, met = ncg_reward_correction(state, args.lambda_ncg, args.lambda_gap)
            shaped = reward + corr
        ...
        st = env.state(); met = spectral_metrics(st)
        ...
        if ep % max(1, args.episodes // 10) == 0:
            print(f"[{ep}] R={row['return']:.3f} E={row['energy']:.3f} smooth={row['ncg_smoothness']:.3f} gap={row['laplacian_gap']:.3f}")
